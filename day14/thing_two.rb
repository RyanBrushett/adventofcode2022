...
class ThingTwo
  ...
  def run_sand
    ...
    until time_to_stop
      grain = SAND_START
      loop do
        if grain[1] == @lowest_rock_point[1] + 1
      ...
      time_to_stop = true if @sand_rest_points.include?(SAND_START)
    end
  end
  ...
  class ThingTwoTest < Minitest::Test
    ...
    def test_the_test_data
      ...
      thing.run_sand
      assert_equal 93, thing.sand_rest_points.count
    end
  end
end
...
thing.parse(input)
thing.run_sand
puts "Part 2: #{thing.sand_rest_points.count}"
