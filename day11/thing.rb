...
class Thing
    ...
    20.times do
      monkeys.each do |monkey|
        while !monkey.items.empty? do
          ...
          new_value = op[0].send(op[1].to_sym, op[2])
          new_value /= 3
  ...
  class ThingTest < Minitest::Test
    ...
    def test_parsing
      ...
      monkeys = thing.monkeys
      assert_equal 4, monkeys.size
      assert_equal 13, monkeys.find { |x| x.id == 2 }.test
      assert_equal 2, monkeys.find { |x| x.id == 1 }.true_case
      assert_equal 3, monkeys.find { |x| x.id == 0 }.false_case
    end
    ...
    def test_run
      ...
      assert_equal 10605, inspect_count.reduce(:*)
    end
  end
end
